...
You might want to include one or more JavaScript libraries in your SharePoint Framework (SPFx) project. This article shows you how to bundle an external library and share libraries.

## Bundle a script

By default, the SPFx's build process's bundler automatically includes any imported library in a SPFx component in the generated bundle. This means that the library is deployed in the same JavaScript bundle file as your web part. This is useful for smaller libraries that aren't used in multiple web parts.

> [!NOTE]
> While this article focuses on a SPFx web part project, but it applies to all types of SPFx components.

### Example

1. Include the string validating library [validator](https://www.npmjs.com/package/validator) package in a web part project.
1. Download the validator package from npm:
    ...
    > [!NOTE]
    > Because you're using TypeScript, you should include the TypeScript type declaration for the package you add. Type declarations make working with JavaScript libraries easier. All the TypeScript code is still converted to JavaScript code when you compile. You can search for and find type declarations by using **npm**, for example: **npm install @types/{package} --save-dev**.

1. Create a file in the web part's folder called **validator.d.ts** and add the following:
...
1. Install the **marked** package into your project:

    ```console
    npm install marked --save
    ```

1. Download the type declarations:

    ```console
    npm install @types/marked --save-dev
    ```

1. Edit the **config/config.json**, and add an entry to the `externals` property. This is what tells the bundler to put this in a separate file. This prevents the bundler from bundling this library:
...
In this example, you'll load the jQuery library from a CDN. You don't need to install the npm package. However, you still need to install the type declarations.
...
1. Update the **./config/config.json** file to load jQuery from CDN. Add an entry to the `externals` property:
...
To load a non-AMD module, you add an additional property to the entry in your **./config/config.json** file.
...
1. Update the **./config/config.json** file to include this script. Add an entry to the `externals` property:
...
Many libraries have dependencies on another library. You can specify such dependencies in the **./config/config.json** file by using the `globalDependencies` property.
...
1. Update the **./config/config.json** file. Add two entries to `externals` property:
...
## Load the SharePoint JSOM

Loading the SharePoint JavaScript Object Model (JSOM) is the same scenario as loading non-AMD scripts that have dependencies. This means using both the `globalName` and `globalDependency` options.
...
You can use the property in the **./config/config.json** file called `localizedResources` to describe how to load localized resources. Paths in this map are relative to the **lib** folder and must not contain a leading slash (**/**).
...
1. Edit the **./config/config.json** file. Add an entry to `localizedResources`. `{locale}` is a placeholder token for the locale name:
